...
class mnist_dataprovider(Sequence):
    def __init__(self, default_boxes_, input_resize=270, batch_size=64, shuffle=True):
        ...
        self.n_sample_train = 5000
        self.n_sample_test = 1000
        (self.train_images, self.train_true, self.train_reg_true), \
        (self.test_images, self.test_true, self.test_reg_true) = \
            self.mnist_localization_generator((84, 84, 3), (84, 84, 3))
    ...
    def random_image_fetcher(self, xs, shape=(84, 84, 3)):
        bg_bucket = []
        coord_bucket = []
        ...
        for sample_img in tqdm(xs[:]):
            ...
            # Generate 84, 84, 3 background image
            # bg = np.random.normal(5, 1, shape)
            bg = np.zeros(shape)

            # Generate random coords
            rand_x = np.random.randint(0, shape[1] - 28 - 1)
            ...
            obj_x_min = obj_x_min * (self.input_resize / 84)
            obj_x_max = obj_x_max * (self.input_resize / 84)
            obj_y_min = obj_y_min * (self.input_resize / 84)
            obj_y_max = obj_y_max * (self.input_resize / 84)
    ...
    def mnist_localization_generator(self, train_shape, test_shape):
        ...
        train_images, train_coords = self.random_image_fetcher(train_xs[:n_sample_train], train_shape)
        train_true = train_ys[:n_sample_train]
        train_reg_true = xyxy2xywh(train_coords)

        test_images, test_coords = self.random_image_fetcher(test_xs[:n_sample_test], test_shape)
        test_true = test_ys[:n_sample_test]
        test_reg_true = xyxy2xywh(test_coords)

        return (train_images, train_true, train_reg_true), (test_images, test_true, test_reg_true)
...
if __name__ == '__main__':
    ...
    inputs, outputs = resnet50_detection_network(input_shape=(270, 270, 3), n_anchors=10, n_classes=10 + 1)
    ...
    dataprovider = mnist_dataprovider(default_boxes_=final_boxes_, input_resize=270, batch_size=64, shuffle=True)
